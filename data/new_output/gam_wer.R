...
df <- read.csv("stats/table_wer.csv", sep="\t")
...
summary(model)
png("wer_my_gam_plots.png", width = 800, height = 600) # Adjust width/height as needed
...
# Open a PNG device
png("wer_gam_check_diagnostics.png", width = 1000, height = 800) # Adjust dimensions
...
# Plot
ggplot(df, aes(x = fit, y = wer, color=data, shape=phase)) +
...
ggsave("plots/wer_predicted_vs_observed.png", width = 7, height = 6, dpi = 300)
...
# Save
ggsave("plots/wer_effects_categorical.png", width = 12, height = 4, dpi = 300)
